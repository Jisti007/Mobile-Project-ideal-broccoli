...
Java_xyz_asdasd_gles3jni_GLES3JNILib_init(JNIEnv *env, jobject obj, jobject assetManager);
JNIEXPORT void JNICALL
...
JNIEXPORT void JNICALL
Java_xyz_asdasd_gles3jni_GLES3JNILib_init(JNIEnv *env, jobject obj, jobject javaAssetManager) {
	JavaVM *vm;
	env->GetJavaVM(&vm);
	vm->AttachCurrentThread(&env, NULL);
	AAssetManager *assetManager = AAssetManager_fromJava(env, javaAssetManager);
	androidAssetManager = std::unique_ptr<AndroidAssetManager>(new AndroidAssetManager(assetManager));
